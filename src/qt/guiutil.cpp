...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent, bool allow_stakeonly)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Darkpay address (e.g. %1)").arg(
...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(!uri.isValid() || uri.scheme() != QString("darkpay"))
        return false;
...
{
    QString ret = QString("darkpay:%1").arg(info.address);
    int paramCount = 0;
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Darkpay.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Darkpay (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Darkpay (%s).lnk", chain);
}
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "darkpay.desktop";
    return GetAutostartDir() / strprintf("darkpay-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Darkpay\n";
        else
            optionFile << strprintf("Name=Darkpay (%s)\n", chain);
