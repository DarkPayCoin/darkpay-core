...
const QString BITCOIN_IPC_PREFIX("darkpay:");
#ifdef ENABLE_BIP70
    ...
    if (startLocalServer)
    {
        ...
        if (!uriServer->listen(name)) {
            ...
            QMessageBox::critical(nullptr, tr("Payment request error"),
                tr("Cannot start darkpay: click-to-pay handler"));
        }
        else {
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("darkpay://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'darkpay://' is not a valid URI. Use 'darkpay:' instead."),
    ...
    else if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // bitcoin: URI
    {
        ...
        // normal URI
        {
            ...
            if (GUIUtil::parseBitcoinURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid Darkpay address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
