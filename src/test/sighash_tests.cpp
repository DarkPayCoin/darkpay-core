...
void static RandomTransaction(CMutableTransaction &tx, bool fSingle) {
    ...
    tx.nVersion = ((uint32_t)InsecureRand32()) % (DARKPAY_TXN_VERSION-1);
...
BOOST_AUTO_TEST_CASE(sighash_from_data)
{
    ...
    for (unsigned int idx = 0; idx < tests.size(); idx++) {
        ...
        bool fExpectHashFailure = false; // adjusting test vectors >= DARKPAY_TXN_VERSION
        try {
          ...
          sigHashHex = test[4].get_str();
            ...
            strHex[0] = raw_tx[0];
            strHex[1] = raw_tx[1];
            if (std::strtoul(strHex, 0, 16) >= DARKPAY_TXN_VERSION)
            {
                raw_tx[0] = '0';
                raw_tx[1] = '0';
