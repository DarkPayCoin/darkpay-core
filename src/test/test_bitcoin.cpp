...
BasicTestingSetup::BasicTestingSetup(const std::string& chainName, bool fDarkpayModeIn)
    : m_path_root(fs::temp_directory_path() / "test_darkpay" / strprintf("%lu_%i", (unsigned long)GetTime(), (int)(InsecureRandRange(1 << 30))))
{
    fDarkpayMode = fDarkpayModeIn;
    ...
    ResetParams(chainName, fDarkpayMode);
...
TestingSetup::TestingSetup(const std::string& chainName, bool fDarkpayModeIn) : BasicTestingSetup(chainName, fDarkpayModeIn)
{
    SetDataDir("tempdir");
