...
BOOST_FIXTURE_TEST_SUITE(darkpaychain_tests, DarkpayBasicTestingSetup)
...
BOOST_AUTO_TEST_CASE(signature_test)
{
    ...
    txn2.nVersion = DARKPAY_TXN_VERSION;
...
BOOST_AUTO_TEST_CASE(darkpaychain_test)
{
    SeedInsecureRand();
    CBasicKeyStore keystore;
    ...
    CBlock blk;
    blk.nVersion = DARKPAY_BLOCK_VERSION;
    blk.nTime = 1487406900;
...
BOOST_AUTO_TEST_CASE(mixed_input_types)
{
    CMutableTransaction txn;
    txn.nVersion = DARKPAY_TXN_VERSION;
    BOOST_CHECK(txn.IsDarkpayVersion());
    ...
    txnPrev.nVersion = DARKPAY_TXN_VERSION;
    BOOST_CHECK(txnPrev.IsDarkpayVersion());
