...
static const uint8_t DARKPAY_BLOCK_VERSION = 0xA0;
static const uint8_t DARKPAY_TXN_VERSION = 0xA0;
static const uint8_t MAX_DARKPAY_TXN_VERSION = 0xBF;
...
inline bool IsDarkpayTxVersion(int nVersion)
{
    return (nVersion & 0xFF) >= DARKPAY_TXN_VERSION;
}
...
inline void SerializeTransaction(const TxType& tx, Stream& s) {
    ...
    if (IsDarkpayTxVersion(tx.nVersion)) {
...
public:
    ...
    static const int32_t CURRENT_DARKPAY_VERSION=0xA0;
...
public:
    ...
    {
        return IsDarkpayTxVersion(nVersion) ? vpout.size() : vout.size();
    }
