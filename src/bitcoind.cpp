 ...
 *
 * This is the developer documentation of the reference client for an experimental new digital currency called Darkpay (https://www.darkdot.network/),
...
static bool AppInit(int argc, char* argv[])
{
    ...
    if (HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        {
            strUsage += "\nUsage:  darkpayd [options]                     Start " PACKAGE_NAME " Daemon\n";
    ...
    try
    {
        ...
        for (int i = 1; i < argc; i++) {
            if (!IsSwitchChar(argv[i][0])) {
                tfm::format(std::cerr, "Error: Command line contains unexpected token '%s', see darkpayd -h for a list of options.\n", argv[i]);
                exit(EXIT_FAILURE);
            }
        }
...
#endif
            tfm::format(std::cout, "Darkpay server starting\n");
