...
bool TransactionCanBeBumped(const CWallet* wallet, const uint256& txid)
{
    ...
    if (fDarkpayMode) {
        const CHDWallet *pw = GetDarkpayWallet(wallet);
        if (!pw) {
            return false;
        }
                         ...
                         CAmount& old_fee, CAmount& new_fee, CMutableTransaction& mtx)
{
    ...
    if (IsDarkpayWallet(wallet)) {
        ...
        if (it != wallet->mapWallet.end()) {
            ...
            if (poutput->GetValue() <= GetDustThreshold((CTxOutStandard*)poutput, GetDiscardRate(*wallet, ::feeEstimator))) {
