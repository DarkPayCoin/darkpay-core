...
int64_t CChainParams::GetCoinYearReward(int64_t nTime) const
 {
     ...
     if (strNetworkID != "regtest") {
        ...
        else if (nYearsSinceGenesis >= 1 && nYearsSinceGenesis < 2) {
            return 18 * CENT;
        }
        else if (nYearsSinceGenesis >= 2 && nYearsSinceGenesis < 3) {
            return 15 * CENT;
        }
        else if (nYearsSinceGenesis >= 3 && nYearsSinceGenesis < 4) {
            return 13 * CENT;
        }
        else if (nYearsSinceGenesis >= 4 && nYearsSinceGenesis < 5) {
            return 5 * CENT;
        }
        else if (nYearsSinceGenesis >= 5 && nYearsSinceGenesis < 6) {
            return 5 * CENT;
        }
        else if (nYearsSinceGenesis >= 6 && nYearsSinceGenesis < 7) {
            return 2.5 * CENT;
        }
        else if (nYearsSinceGenesis >= 7 && nYearsSinceGenesis < 8) {
            return 2.5 * CENT;
        }
        else  {
            return 2 * CENT;
...
const std::pair<const char*, CAmount> genesisOutputsTestnet[] = {
     std::make_pair("e435137d93a55098359766afafe06a996fbbae61",36000 * COIN), // dev
     std::make_pair("27824fb0539e64ac39623453a421f5c63a2ca575",1050000 * COIN), // swap1
...
static CBlock CreateGenesisBlockRegTest(uint32_t nTime, uint32_t nNonce, uint32_t nBits)
{
    ...
    genesis.nBits    = nBits;
    genesis.nNonce   = nNonce;
    genesis.nVersion = DARKPAY_BLOCK_VERSION;
    genesis.vtx.push_back(MakeTransactionRef(std::move(txNew)));
...
consensus.nSubsidyHalvingInterval = 210000;
        ...
        consensus.nMinimumChainWork = uint256S("0x00000000000000000000000000000000000000000000003526cdcc4b735b80b2"); // 435056
        ...
        base58Prefixes[EXT_PUBLIC_KEY]     = {0x3a, 0x80, 0x58, 0x37}; // DRKP
        base58Prefixes[EXT_SECRET_KEY]     = {0x3a, 0x80, 0x61, 0xa0}; // DRKV
        ...
        checkpointData = {
            {
               ...
               { 250000,    uint256S("0xf8438121dc0757ee70b26fca5eb76e1fbe1058cf09f8fffdbf7819a75fb3cf8a")},
               { 300000,    uint256S("0x9a68c1f32335c18a3d43987d34d0e83bf52bbcf3d0c5242c1d94f008a2405e1b")},
               { 350000,    uint256S("0x95f3f737423fcaf2312bef1130df146c6af948f57c5672abffa1dc0a9e2f1c50")},
               { 400000,    uint256S("0x18ab1ee2b59e56e36defa6f656631abce53fa28642338220b11c24e833e9fdad")},
               { 435056,    uint256S("0xaf9fb968d5f0d51ca79f3328e0f58f1adcb0d59151c9ad9acbae9587a345f03d")},
