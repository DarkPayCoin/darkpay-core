Darkpay Core version 0.16.0.1 is now available.
...
Darkpay Core 0.16 introduces generic support for Hardware Signing Devices to provide key-signing capabilities. 
...
In order to facilitate deterministic generation of stealth addresses between Darkpay Core and Signing Devices the method used to generate these addresses has been changed.
...
Darkpay Core now has more flexibility in where the wallets directory can be
located. Previously wallet database files were stored at the top level of the
darkpay data directory. The behavior is now:
...
------------------------------------
The minimum version of the GCC compiler required to compile Darkpay Core is now 4.8. No effort will be
...
- darkpay-cli now has an `-stdinrpcpass` option to allow the RPC password to be read from standard input.
- darkpay-cli now supports a new `-getinfo` flag which returns an output like that of the now-removed `getinfo` RPC.
...
- #11970 `b7450cd` Add test coverage for darkpay-cli multiwallet calls (ryanofsky)
...
- #10997 `affe927` Add option -stdinrpcpass to darkpay-cli to allow RPC password to be read from standard input (jharvell)
...
- #11621 `1f7695b` [build] Add temp_darkpay_locale_qrc to CLEAN_QT to fix make distcheck (fanquake)
...
- #11711 `6378e5c` darkpay_qt.m4: Minor fixes and clean-ups (fanquake)
...
- #11297 `16e4184` Make sure ~/.darkpay doesn't exist before build (MeshCollider)
...
- #11143 `3aa60b7` Fix include path for darkpay-config.h (danra)
...
- #10871 `c9a4aa8` Handle getinfo in darkpay-cli w/ -getinfo (revival of #8843) (achow101)
- #11419 `093074b` Utils: Fix launchctl not being able to stop darkpayd (OmeGak)
...
- #10529 `331352f` Improve darkpayd systemd service file (Flowdalic)
