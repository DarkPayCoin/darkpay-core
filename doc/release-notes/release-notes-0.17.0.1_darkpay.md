Darkpay Core version 0.17.0.1 is now available:
...
- `-includeconf=<file>` can be used to include additional configuration files.
  Only works inside the `darkpay.conf` file, not inside included files or from
  ...
  as darkpay.conf will still include `relative.conf`.
...
Previously, wallets could only be loaded or created at startup, by specifying `-wallet` parameters on the command line or in the darkpay.conf file. It is now possible to load, create and unload wallets dynamically at runtime:
...
prefixing the option with the network, such as:
    ...
    main.uacomment=darkpay
    test.uacomment=darkpay-testnet
...
be used in V0.17 by starting darkpayd with the '-deprecatedrpc=accounts'
...
BIP 174 Partially Signed Darkpay Transactions support
-----------------------------------------------------

[BIP 174 PSBT](https://github.com/bitcoin/bips/blob/master/bip-0174.mediawiki) is an interchange format for Darkpay transactions that are not fully signed
...
Overall, the construction of a fully signed Darkpay transaction goes through the
following steps:
...
- An **Extractor** produces a valid Darkpay transaction (in network format)
...
- The new RPC `getzmqnotifications` returns information about active ZMQ
  notifications.
- When darkpay is not started with any `-wallet=<path>` options, the name of
  the default wallet returned by `getwalletinfo` and `listwallets` RPCs is
  now the empty string `""` instead of `"wallet.dat"`. If darkpay is started
...
- `signrawtransaction` is deprecated and will be fully removed in v0.18. To use
  `signrawtransaction` in v0.17, restart darkpayd with
...
- When running darkpayd with `-debug` but without `-daemon`, logging to stdout
...
meaning the `-txindex` flag can be toggled without a full reindex. If darkpayd
...
- #12723 `310dc61` Qt5: Warning users about invalid-BIP21 URI darkpay:// (krab)
...
- #13528 `b877c39` Move DarkpayGUI initializers to class, fix initializer order warning (laanwj)
...
- #13604 `75bea05` Add depends 32-bit arm support for darkpay-qt (TheCharlatan)
...
- #13696 `2ab7208` Add aarch64 qt depends support for cross compiling darkpay-qt (TheCharlatan)
...
- #11771 `2dbc4a4` Change invalidtxrequest to use DarkpayTestFramework (jnewbery)
...
- #12443 `07090c5` Move common args to darkpay.conf (MarcoFalke)
...
- #11772 `0630974` Change invalidblockrequest to use DarkpayTestFramework (jnewbery)
...
- #11773 `f0f9732` Change `feature_block.py` to use DarkpayTestFramework (jnewbery)
- #12839 `40f4baf` Remove travis checkout depth (laanwj)
- #11817 `2a09a78` Change `feature_csv_activation.py` to use DarkpayTestFramework (jnewbery)
...
- #12856 `27278df` Add Metaclass for DarkpayTestFramework (WillAyd)
...
- #12882 `d96bdd7` Make `test_darkpay` pass under ThreadSanitzer (clang). Fix lock-order-inversion (potential deadlock) (practicalswift)
...
- #13687 `9d26b69` travis: Check that ~/.darkpay is never created (MarcoFalke)
...
- #13811 `660abc1` travis: Run `bench_darkpay` once (MarcoFalke)
...
- #12102 `7fb8fb4` Apply hardening measures in darkpayd systemd service file (Flowdalic)
- #12652 `55f490a` darkpay-cli: Provide a better error message when darkpayd is not running (practicalswift)
...
- #12769 `35eb9d6` Add systemd service to darkpayd in debian package (ghost)
...
- #13374 `56f6936` utils and libraries: checking for darkpay address in translations (kaplanmaxe)
...
- #13603 `453ae5e` darkpay-tx: Stricter check for valid integers (domob1812)
...
- #13725 `07ce278` Fix darkpay-cli --version (Empact)
...
- #12877 `2b54155` Use darkpayd in Tor documentation (knoxcard)
...
- #13905 `73a09b4` fixed darkpay-cli -help output for help2man (hebasto)
...
Thanks to the Core Developers at Darkpay and Bitcoin (https://github.com/bitcoin/bitcoin).
