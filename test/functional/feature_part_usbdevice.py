...
from test_framework.test_darkpay import (
    DarkpayTestFramework,
    isclose,
    getIndexAtProperty,
)
...
class USBDeviceTest(DarkpayTestFramework):
    ...
    def run_test(self):
        ...
        # Check that darkpay has been built with USB device enabled
        ...
        if not config["components"].getboolean("ENABLE_USBDEVICE"):
            raise SkipTest("darkpayd has not been built with usb device enabled.")
