...
def main():
    ...
    tmpdir = "%s/darkpay_test_runner_₿_🏃_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    ...
    run_tests(
        ...
        runs_ci=args.ci,
        create_cache=(True if args.bitcoin or (not args.darkpay and not args.insight) else False)
    )
...
def run_tests(test_list, src_dir, build_dir, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0, failfast=False, runs_ci=False, create_cache=True):
    ...
    try:
        if subprocess.check_output(["pidof", "darkpayd"]) is not None:
            print("%sWARNING!%s There is already a darkpayd process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
